...
force_generate=0
generatedversions="$HOME/.mlw4/wallpaper-generated"
waypaperrunning=$HOME/nix/assets/cache/waypaper-running
cachefile="$HOME/nix/assets/cache/current_wallpaper"
blurredwallpaper="$HOME/.mlw4/wallpaper-current/blurred_wallpaper.png"
squarewallpaper="$HOME/.mlw4/wallpaper-current/square_wallpaper.png"
rasifile="$HOME/nix/assets/cache/current_wallpaper.rasi"
blurfile="$HOME/nix/assets/settings/blur.sh"
defaultwallpaper="$HOME/wallpaper/default.jpg"
wallpapereffect="$HOME/nix/assets/settings/wallpaper-effect.sh"
blur="50x30"
blur=$(cat $blurfile)
...
if [ -f $wallpapereffect ]; then
    ...
    if [ ! "$effect" == "off" ]; then
        ...
        if [ -f $generatedversions/$effect-$wallpaperfilename ] && [ "$force_generate" == "0" ] && [ "$use_cache" == "1" ]; then
            ...
            source $HOME/nix/assets/hypr/effects/$effect
        fi
...
#set theme for discord
~/nix/scripts/discord.sh

#set theme for kitty permanently
~/nix/scripts/kitty.sh
