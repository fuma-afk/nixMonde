...
if [[ "$1" == "rofi" ]]; then
    ...
    # Open rofi
    choice=$(echo -e "$options" | rofi -dmenu -replace -config ~/nix/home-manager/modules/rofi/ml4w/config-hyprshade.rasi -i -no-show-icons -l 4 -width 30 -p "Hyprshade") 
    if [ ! -z $choice ] ;then
        echo "hyprshade_filter=\"$choice\"" > ~/nix/assets/settings/hyprshade.sh
    ...
    if [ -f ~/nix/assets/settings/hyprshade.sh ] ;then
        source ~/nix/assets/settings/hyprshade.sh
    fi
